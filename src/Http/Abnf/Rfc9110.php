...
readonly class Rfc9110
{
	...
	// RFC 9110 §12.4.2 — qvalue = ( "0" [ "." *3DIGIT ] ) / ( "1" [ "." *3"0" ] )
	...
	public const MEDIA_TYPE = '(?<media_type>' . self::TYPE . '/' . self::SUBTYPE . self::PARAMETERS . ')';
	...
	// RFC 9110 §12.4.2 — weight = OWS ";" OWS "q=" qvalue
	public const WEIGHT = self::OWS . ';' . self::OWS . '(?:Q|q)=(?<q>' . self::QVALUE . ')';

	// RFC 9110 §12.5.1 — media-range = ( "*/*" / ( type "/*" ) / ( type "/" subtype ) ) parameters
	public const MEDIA_RANGE = '(?<media_range>(?:\*/\*|(?:' . self::TYPE . '/\*)|(?:' . self::TYPE . '/' . self::SUBTYPE . '))' . self::PARAMETERS . ')';

	// RFC 9110 §12.5.1 — Accept = [ ( media-range [ weight ] ) *( OWS "," OWS ( media-range [ weight ] ) ) ]
